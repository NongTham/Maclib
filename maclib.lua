--[[
    ...
    ║           Originally by @biggaboy212 | Upgraded & Enhanced        ║
    ╚═══════════════════════════════════════════════════════════════════╝
    Features:
      - Native Mobile & Tablet Support with Auto-Responsive Scaling
      - Draggable Mobile Floating Toggle Button
      - Lucide Icons & Spritesheet Support (ImageRectOffset / Size)
      - Dynamic Theme & Accent Color System
      - 100% Memory Leak Free with Complete Maid Cleanup on Unload
      - Fixed Keybind Blacklist Crash & UpdateName Bug
      - Fixed Config System Toggle 'false' Value Loading Bug
      - Clickable & Draggable Slider Bar with Clamped Width
      - Dropdown Max Height Constraint with Smooth Scrolling
      - 100% Backward Compatible with Existing Maclib Scripts
]]
...
--// Helper: Maid Utility for Leak-Free Resource Management
local function CreateMaid()
	local maid = { _connections = {}, _instances = {} }
	function maid:Give(task)
		if typeof(task) == "RBXScriptConnection" then
			table.insert(self._connections, task)
		elseif typeof(task) == "Instance" then
			table.insert(self._instances, task)
		end
		return task
	end
	function maid:Clean()
...
--// Helper: Universal Icon / Lucide Spritesheet Resolver
local function ResolveIcon(imageLabel, icon, rectOffset, rectSize)
	if not imageLabel or not icon then return end
	if typeof(icon) == "table" then
		local img = icon.Url or icon.Image or icon.id or icon.Id or icon.AssetId
		local off = icon.ImageRectOffset or icon.rectOffset or icon.Offset or icon.offset or rectOffset
		local sz = icon.ImageRectSize or icon.rectSize or icon.Size or icon.size or rectSize
		if img then imageLabel.Image = img end
		if off then imageLabel.ImageRectOffset = off end
	...
	if typeof(icon) == "string" then
		...
		if builtIn then
			imageLabel.Image = builtIn.Image
			imageLabel.ImageRectOffset = builtIn.Offset or Vector2.zero
			imageLabel.ImageRectSize = builtIn.Size or Vector2.zero
			return
		end
		imageLabel.Image = icon
...
local function ResolveColor(val, default)
	if typeof(val) == "Color3" then return val end
	if type(val) == "string" and MacLib.Themes[val] then return MacLib.Themes[val] end
	return default or Color3.fromRGB(0, 122, 255)
end
...
function MacLib:Window(Settings)
	...
	WindowFunctions.Maid = CreateMaid()
	WindowFunctions.AccentColor = ResolveColor(Settings.AccentColor, Color3.fromRGB(0, 122, 255))
	...
	local macLib = GetGui()
	WindowFunctions.ScreenGui = macLib
	...
	--// Auto-Responsive Mobile Scaling
	local userScale = Settings.Scale or 1
	local function CheckViewportScale()
		if Settings.AutoAdjustScale == false then
		...
		local pad = 24
		if vp.X < (baseW + pad) or vp.Y < (baseH + pad) then
			local fitScale = math.clamp(math.min((vp.X - pad) / baseW, (vp.Y - pad) / baseH), 0.42, 1.0)
			baseUIScale.Scale = fitScale * userScale
		else
			baseUIScale.Scale = userScale
		end
	end
	...
	do
		...
		local canContinue = IsNotNaN(camera:ScreenPointToRay(0,0).Origin.x)
		while not canContinue do
			RunService.RenderStepped:Wait()
			canContinue = IsNotNaN(camera:ScreenPointToRay(0,0).Origin.x)
		end
	end
	...
	WindowFunctions.Maid:Give(RunService.RenderStepped:Connect(UpdateOrientation))
	...
	function WindowFunctions:TabGroup()
		...
		function SectionFunctions:Tab(Settings)
			...
			if Settings.Image or Settings.Icon then
				...
				ResolveIcon(tabImage, Settings.Image or Settings.Icon, Settings.ImageRectOffset, Settings.ImageRectSize)
			...
			function TabFunctions:SetName(newName)
				tabSwitcherName.Text = newName
			end
			function TabFunctions:SetImage(newImage, newOffset, newSize)
				...
				ResolveIcon(tabImage, newImage, newOffset, newSize)
			end
			function TabFunctions:SetBadge(badgeText)
				if not tabBadge then
					tabBadge = Instance.new("TextLabel")
					tabBadge.Name = "TabBadge"
					tabBadge.FontFace = Font.new(assets.interFont, Enum.FontWeight.SemiBold, Enum.FontStyle.Normal)
					tabBadge.TextColor3 = Color3.fromRGB(255, 255, 255)
					tabBadge.TextSize = 11
					tabBadge.BackgroundColor3 = WindowFunctions.AccentColor
					tabBadge.BorderSizePixel = 0
					tabBadge.AutomaticSize = Enum.AutomaticSize.XY
					tabBadge.LayoutOrder = 2
					local bCorner = Instance.new("UICorner")
					bCorner.CornerRadius = UDim.new(1, 0)
					bCorner.Parent = tabBadge
					...
					bPad.Parent = tabBadge
					tabBadge.Parent = tabSwitcher
				end
				if badgeText and tostring(badgeText) ~= "" then
					tabBadge.Text = tostring(badgeText)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					local function NewState(State, callback)
						...
						Tween(toggle1, TweenSettings.Info, {
							ImageColor3 = State and WindowFunctions.AccentColor or Color3.fromRGB(87, 86, 86)
						}):Play()
						if callback then
							callback(togglebool)
						end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					function KeybindFunctions:Bind(Key)
						...
						binderBox.Text = (typeof(Key) == "EnumItem" and Key.Name) or tostring(Key)
					end
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					function DropdownFunctions:ClearOptions()
						for _, optionData in pairs(OptionObjs) do
							pcall(function() optionData.Button:Destroy() end)
						end
						OptionObjs = {}
						Selected = {}
					...
					function DropdownFunctions:SetValues(newOptions, newSelection)
						DropdownFunctions:ClearOptions()
						DropdownFunctions:InsertOptions(newOptions)
						if newSelection ~= nil then
							DropdownFunctions:UpdateSelection(newSelection)
						end
					end

					function DropdownFunctions:Refresh(newOptions)
						local prev = DropdownFunctions.Value
						DropdownFunctions:ClearOptions()
						DropdownFunctions:InsertOptions(newOptions)
						if prev then
							DropdownFunctions:UpdateSelection(prev)
						end
					end
	...
	WindowFunctions.Maid:Give(UserInputService.InputEnded:Connect(function(inp, gpe)
	...
	function WindowFunctions:SetAccentColor(color)
		WindowFunctions.AccentColor = ResolveColor(color, Color3.fromRGB(0, 122, 255))
	end
	...
	local ClassParser = {
		...
		["Keybind"] = {
			Save = function(Flag, data)
				local b = data.Bind or (data.GetBind and data:GetBind())
				return {
					type = "Keybind", 
					flag = Flag, 
					bind = (typeof(b) == "EnumItem" and b.Name) or (type(b) == "string" and b) or nil
				}
			end,
			Load = function(Flag, data)
	...
	WindowFunctions.SaveConfig = function(_, path) return MacLib:SaveConfig(path) end
	WindowFunctions.LoadConfig = function(_, path) return MacLib:LoadConfig(path) end
	WindowFunctions.RefreshConfigList = function(_) return MacLib:RefreshConfigList() end
	WindowFunctions.LoadAutoLoadConfig = function(_) return MacLib:LoadAutoLoadConfig() end
	WindowFunctions.SetFolder = function(_, folder) return MacLib:SetFolder(folder) end
